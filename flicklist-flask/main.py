...
def get_current_watchlist():
    unwatched = Movie.query.filter_by(watched=False).all()
    return unwatched.query.filter_by(user_id=user.id).all()

def get_watched_movies():
    watched = Movie.query.filter_by(watched=True).all()
    return watched.query.filter_by(user_id=user.id).all()
...
def register():
    if request.method == 'POST':
        ...
        if password != verify:
            ...
            return redirect('/register')
        user = User(email=email, password=make_pw_hash(password))
        db.session.add(user)
        db.session.commit()
        session['user'] = user.email
        return redirect("/")
...
def add_movie():
    ...
    movie = Movie(name=new_movie_name, owner_id=user.id)
    db.session.add(movie)
    db.session.commit()
